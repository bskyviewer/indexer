...
- `q`:
  [lucene query](https://lucene.apache.org/core/10_2_2/queryparser/org/apache/lucene/queryparser/classic/package-summary.html#terms-heading).
  the following fields can be searched:
    ...
    - `createdAt`: utc creation date—as these are lexically ordered, you can filter date ranges using lucene range
      syntax,
    ...
    - `label`: labels applied to the post. currently only
      [self-labels](https://docs.bsky.app/docs/advanced-guides/moderation#self-labels) are indexed
    - `has`: convenience field for checking if the post has an `embed`, `tag` or `label`. for example, you can exclude
      posts with embedded content from your search result by adding `-has:embed` to the query
